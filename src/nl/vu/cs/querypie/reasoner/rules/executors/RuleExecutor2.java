...
public class RuleExecutor2 extends RuleExecutor1 {
	...
	// triples_derived_prev_cycle = null;
	...
	if (n_vars_others_output > 0) {
	    ...
	    if (shared_vars_precomps_last_generic.length == 2) {
		key.values[1] = t
	    ...
	    if (col != null) {
		int pos = vars_others_output[0].pos2;
		for (int y = 0; y < col.getNTuples(); ++y) {
		    k3.values[pos] = col.getValue(y, 0);
		    ...
		    triple[pos].setValue(col.getValue(y, 0));
