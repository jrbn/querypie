...
public class Buckets {
	...
	private Map<Long, TransferInfo>[] activeTransfers;
	...
	public synchronized void clearSubmission(int submissionId) {
		if (buckets.size() > 0) {
			...
			if (count > 0) {
				log.warn("There were still " + count + "  buckets unused of a total size of " + size);
			}
		}
	}
	...
	public synchronized Bucket getOrCreateBucket(int submissionNode,
			int idSubmission, String sortingFunction, Object[] sortingParams) {
	...
	public TupleIterator getIterator(int submissionNode, int idSubmission,
			int idBucket) {
		Bucket bucket = getOrCreateBucket(submissionNode, idSubmission, idBucket,
				null, null);
		BucketIterator itr = new BucketIterator();
		itr.init(bucket, idSubmission, idBucket, this);
		return itr;
	...
	public Bucket startTransfer(int submissionNode, int submission, int node,
			int bucketID, String sortingFunction, Object[] sortingParams) {
	...
	public void finishTransfer(int submissionNode, int submission, int node,
			...
			Object[] sortingParams, boolean decreaseCounter) throws IOException {
		...
		if (sortingFunction == null || sortingFunction.equals("")) {
			...
			if (sortingParams != null && sortingParams.length > 0) {
				...
				Utils.writeObjectsOnMessage(message, sortingParams);
			} else {
				message.writeInt(0);
			}
		}
